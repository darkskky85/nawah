...
def is_empty_summary(summary):
    """يكشف الملخّصات الفارغة معنوياً (عبارات اعتذار أو نائبة) لاستبعادها."""
    s = (summary or "").strip()
    if len(s) < 25:
        return True
    bad = [
        "لا توجد معلومات", "لا تتوفر معلومات", "لا يمكن التلخيص", "لا يمكن تلخيص",
        "معلومات كافية", "السياق المقدم", "السياق المقدّم", "غير كافية لتلخيص",
        "لا أستطيع", "لا يوجد محتوى", "بدقة.", "لتلخيص هذا الخبر",
    ]
    low = s.replace("ـ", "")
...
def summarize_arabic(api_key, batch):
    ...
    system = (
        ...
        "إذا كان العنصر عن أداة ذكاء اصطناعي جديدة (غالباً من Product Hunt)، فاكتب جملتين تعريفيتين "
        "محايدتين عن وظيفة الأداة المرجّحة بناءً على اسمها ومجالها، تبدأ بـ«أداة جديدة ظهرت باسم …»، "
        "دون مدح أو ترويج أو ادعاء تفوّق أو أرقام. "
        "مهم جداً: لا تكتب أبداً عبارات اعتذار مثل «لا توجد معلومات كافية» أو «لا يمكن التلخيص». "
        "إن كنت لا تعرف الأداة أو الخبر إطلاقاً ولا تستطيع كتابة وصف محايد مفيد، فاترك الحقل summary فارغاً (\"\")."
    )
    user = (
        "لكل خبر مرقّم أدناه، اكتب:\n"
        "- \"ar_title\": عنوان عربي أصلي واضح (لا ترجمة حرفية).\n"
        "- \"summary\": ملخّص عربي أصلي في جملتين إلى ثلاث (30–60 كلمة). للأخبار وضّح الخبر وأهميته؛ "
        "وللأدوات الجديدة عرّف بوظيفة الأداة بحياد. وإن تعذّر ذلك تماماً اترك summary فارغاً \"\".\n\n"
    ...
    for i in range(0, len(fresh), 6):
        ...
        for b, s in zip(batch, out):
            ...
            if is_empty_summary(summary):
                log(f"تخطّي بطاقة بلا محتوى كافٍ: {ar_title[:40]}")
                continue
            results.append({
                "title": ar_title,
                "summary": summary,
